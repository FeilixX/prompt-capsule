...
const ZH: Dict = {
	...
	sk_url_path: '路径',
	sk_url_code: '卡带码',
	sk_url_codehint: '↑ 只发这段就行',
	sk_urlmap_p:
		'一条卡带链接长这样。协议、域名、路径都是固定的，真正认出这盘卡带的，是最后那一小段 ——「卡带码」。所以不用发整条链接，把这段码单独发出去，装了 skill 的 AI 一样能取回。',
	sk_flow1_b: '一段提示词',
	...
	sk_th_args: '参数',
	sk_th_ret: '返回',
	sk_read_args: 'target（编码 或 URL 都吃）',
	sk_read_ret: '卡带正文（过期/无效则告知）',
	sk_k4: '一个完整例子',
	sk_h2_4: '建 → 分享 / 让下游 fetch → 删。',
	sk_ex_c1: '// 1. agent 调 create，返回：',
	sk_ex_human: '// 给人看',
	sk_ex_agent: '// 给 agent fetch',
	sk_ex_code: '// 卡带编码，单独发也能用',
	sk_ex_share: '// 只发编码的分享串（无 URL，防降权）',
	sk_ex_private: '// 私藏，别公开',
	sk_ex_c2: '// 2. 下游 agent fetch raw_url（或只给编码，让它 read_prompt_tape），拿正文照做',
...
const EN: Dict = {
	...
	sk_url_scheme: 'scheme',
	sk_url_domain: 'domain',
	sk_url_path: 'path',
	sk_url_code: 'the code',
	sk_url_codehint: '↑ share just this',
	sk_urlmap_p:
	...
	sk_th_args: 'Args',
	sk_th_ret: 'Returns',
	sk_read_args: 'target (code or URL)',
	sk_read_ret: "the tape's text (or tells you it's gone/invalid)",
	...
	sk_ex_code: '// the capsule code — shareable on its own',
	sk_ex_share: '// code-only share line (no URL, dodges downranking)',
	sk_ex_private: '// keep private',
	sk_ex_c2: '// 2. downstream fetches raw_url (or just the code via read_prompt_tape), runs the text',
